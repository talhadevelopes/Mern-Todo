...
Components: Dockerfile (instructions), Image (built app), Container (running instance), Registry (e.g., Docker Hub).
...
docker build -t talhadevelopes/mern-todo-frontend:06 .
Builds frontend image from Dockerfile, tags as :06.


docker build -t talhadevelopes/mern-todo-backend:06 .
Builds backend image, tags as :06.
...
docker images
Lists images (e.g., talhadevelopes/mern-todo-frontend:06).


docker rmi talhadevelopes/mern-todo-frontend:06
Deletes an image.


docker tag mern-todo-frontend:06 talhadevelopes/mern-todo-frontend:06
Renames/tags image for Docker Hub.
...
docker run -d --rm --name mern-todo-frontend -p 5173:5173 --env-file .env talhadevelopes/mern-todo-frontend:06
Runs frontend container, maps port 5173, loads .env, auto-removes.


docker run -d --rm --name mern-todo-backend -p 3006:3006 --env-file .env talhadevelopes/mern-todo-backend:06
Runs backend container, maps port 3006.


docker run -it talhadevelopes/mern-todo-frontend:06
Runs container interactively for debugging.
...
docker stop mern-todo-frontend
Stops a container.


docker rm mern-todo-frontend
Deletes a stopped container.


docker logs mern-todo-frontend
Views container logs.


docker restart mern-todo-frontend
...
docker push talhadevelopes/mern-todo-frontend:06
Pushes image to Docker Hub.


docker pull talhadevelopes/mern-todo-frontend:06
...
docker run -d --rm -v myvolume:/app mern-todo-frontend:06
Mounts named volume myvolume to /app.


docker run -d --rm -v C:\Users\Talha Ahmed\Desktop\Projects\Mern-Todo\frontend:/app mern-todo-frontend:06
Bind mounts local directory to /app.
...
Removes all unused containers, images, networks (use cautiously).
...
Use specific base image tags (e.g., node:18).

Minimize layers (combine COPY, RUN).

Add .dockerignore to exclude node_modules, .env.
...
Views logs for frontend service.
...
docker-compose stop
Stops services without removing.
...
Set container_name for clarity.

Use env_file for .env variables.

Add healthcheck for reliability.

Use depends_on with condition: service_healthy.

Define custom network or use default.

Add restart: unless-stopped for production.
...
Edit Code: Update frontend/src or backend/src.

Update docker-compose.yml: Increment image tags (e.g., :06 to :07).
...
Test: Open http://localhost:5173, check logs:
docker-compose logs frontend
...
docker network ls
Lists networks (e.g., mern-network).
...
docker run --network mern-network
Runs container on specific network.


docker exec -it mern-todo-frontend bash
Opens shell in running container.


docker inspect mern-todo-frontend
...
Interview Tip: Explain flow (Dockerfile → Image → Container), emphasize .dockerignore, health checks, and multi-stage builds.
